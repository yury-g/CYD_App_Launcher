...
## Unreleased
- 2026-05-24 App 4 manual scan crash-control pass: changed Pin Scanner to start inactive, list `GPIO35`, `GPIO22`, `GPIO21`, and `GPIO27`, and scan only one tapped pin at a time; `GPIO21` and `GPIO22` are guarded with status text instead of `analogRead()` calls. Reordered app navigation so Settings follows Pulse, Pin Scanner follows Settings, the `your app here` page is second-to-last, and Origin Story is always the final app; added a Settings `Memory` free-heap row. Firmware version is `0.4.10-manual-pin-scanner`.
