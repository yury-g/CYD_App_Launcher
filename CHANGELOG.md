...
## Unreleased
- 2026-05-26 app-shell p2p human-range integration: branched from the known-good `0.4.24-front-id` app-shell backup and transplanted the successful `0.5.15-humanrange` Pulse behavior into it; active Pulse acquisition is now p2p-first with 60-200 BPM / 300-1000 ms timing, tight p2p score, three accepted beats to lock, graph markers for p2p/strict/locked/reject/artifact events, conservative default-off audio, and SignalLab fixture checks; firmware version is `0.5.17-p2papps`.
