...
- 2026-05-24 first-screen version identity: added `APP_VERSION` and `APP_FIRMWARE_DATE` directly under `PulseSensor.com` on the Pulse dashboard header, updated the render mock, and added a guard so future experiment branches keep the version/date visible on the first screen for hardware testing; firmware version was `0.4.24-front-id`.
