...
- Made raw CSV logging an internal diagnostic build mode: `pio run -e cyd` is the quieter release build, while `pio run -e cyd_diag` enables 50 Hz `rawDiag` rows and reports version `0.4.25-stable-wave-log`.
...
- Stabilized the live waveform drawing: the graph uses a fixed ADC viewport instead of rolling auto-scale, and beat effects no longer draw circles on top of the raw trace. BPM/IBI qualification and diagnostics still use the live rolling range.
