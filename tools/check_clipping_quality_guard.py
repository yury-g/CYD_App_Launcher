...
required = {
    ...
    '#define APP_VERSION "0.4.25-stable-wave"': "firmware version should identify the stable waveform build",
}

missing = [message for token, message in required.items() if token not in source]

platformio = Path("platformio.ini").read_text()
if '-D APP_VERSION=\\"0.4.25-stable-wave-log\\"' not in platformio:
    missing.append("diagnostic PlatformIO env should report the stable waveform logger version")
