...
def draw_display_row(draw, y, p, mode_variant):
    draw.text((10, y + 2), "Display", font=FONT_SETTINGS, fill=p["label_text"])
    draw.text((10, y + 22), p["mode_value"], font=FONT_SETTINGS, fill=p["value_text"])
    if mode_variant == "cycle":
        draw_button(draw, 226, y + 8, 90, p["name"], p, True)
    else:
        labels = [
            ("MD", "mono_dark"),
            ("ML", "mono_light"),
        ...
        for label, mode in labels:
            draw_button(draw, x, y + 8, 38, label, p, active=mode == p["key"])
            x += 40
...
def draw_settings_picker(mode_key, mode_variant):
    ...
    for label, value in rows:
        draw.text((10, y + 2), label, font=FONT_SETTINGS, fill=p["label_text"])
        draw.text((10, y + 22), value, font=FONT_SETTINGS, fill=p["value_text"])
        draw_dotted_line(draw, (0, y + SETTINGS_ROW_H - 2), (320, y + SETTINGS_ROW_H - 2), p["row_line"], 5, 2)
        y += SETTINGS_ROW_H
    ...
    draw.text((10, y + 2), "WiFi", font=FONT_SETTINGS, fill=p["label_text"])
    draw.text((10, y + 22), "setup later", font=FONT_SETTINGS, fill=p["value_text"])
    draw_dotted_line(draw, (0, y + SETTINGS_ROW_H - 2), (320, y + SETTINGS_ROW_H - 2), p["row_line"], 5, 2)
